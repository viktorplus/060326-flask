# l07_orm_aggregates — схема проекта
...
```
cd l07_orm_aggregates
python app.py
```

Имя каталога — корректный идентификатор Python, поэтому `import l07_orm_aggregates.app` возможен
(`__init__.py` для этого и лежит рядом). Но запускать нужно всё равно из каталога проекта — см. ниже.

`create_engine('sqlite:///db.sqlite')` — путь относительный. Готовый `db.sqlite` лежит внутри
`l07_orm_aggregates/`, поэтому запускать нужно из этого каталога.
...
flowchart TB
    ...
    subgraph models["Модели — те же, что в l05_orm_relationships"]
...
Схема БД совпадает со схемой в [`l05_orm_relationships`](../l05_orm_relationships/SCHEMA.md) (там же — фильтры
`WHERE` и сортировки). Продолжение темы — [`l08_orm_loading`](../l08_orm_loading/SCHEMA.md): `HAVING`,
...
* **Неиспользуемые импорты удалены**: `label`, `and_`, `or_`, `not_`, `desc` — все они остались
  от прошлого урока и в запросах этого файла не участвуют. Присвоение имени вычисляемой колонке
  (`.label('addr_count')`) разбирается в [`l08_orm_loading`](../l08_orm_loading/SCHEMA.md).
