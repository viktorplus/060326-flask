...
```
python -m flask_04_09_26.app
```

Запускать **из корня проекта**, и только так:

* внутри модулей относительные импорты (`from .schemas import ...`) — они требуют запуска пакетом;
* `DATA_FILE = "flask_04_09_26/employees_data.json"` — путь относительный и разрешается от
  текущего рабочего каталога, а не от расположения `settings.py`. Из другого каталога файл
  будет создан не там, где ожидается.
...
* **`.env` и код читают разные ключи.** В `app.py` стоит `os.environ.get("DB_USERNAME")`, а в
  корневом `.env` лежит ключ `USERNAME`. Значит `DB_USERNAME` сейчас `None`. На работу урока это
  не влияет — переменная нигде дальше не используется (`conn_str = f'{DRIVER}'` — заготовка на
  будущий урок с настоящей БД), но если на неё начнут опираться, это сломается молча.
