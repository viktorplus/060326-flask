...
каталога. Готовый файл `db.sqlite` лежит **внутри** `flask_09_09_26/`, поэтому запускать нужно
...
Схема БД в этом файле совпадает со схемой в [`10_09_26`](../10_09_26/SCHEMA.md) (те же `users`
и `addresses` с `description`), но **отличается** от [`11_09_26`](../11_09_26/SCHEMA.md), где
...
* **`user_first.id` без проверки на `None`.** `.first()` на пустой таблице вернёт `None`, и
  следующая же строка упадёт с `AttributeError`. То же с `.one()` — при пустой базе будет
  `NoResultFound`. Скрипт работает только потому, что `db.sqlite` уже наполнен.
* **`==` внутри `.where()` — это не сравнение Python**, а построение SQL-условия `id = 1`.
  По той же причине нельзя писать питоновские `and` / `or` / `not` — нужны `and_()`, `or_()`,
  `not_()`, иначе выражение вычислится в Python и в SQL уйдёт не то.
...
* `func` импортирован, но в этом файле не используется — агрегаты начинаются в
  [`10_09_26`](../10_09_26/SCHEMA.md). То же с `Optional` и `random`: они нужны только
  закомментированным блокам.
