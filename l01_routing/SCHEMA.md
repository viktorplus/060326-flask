...
* **Конкуренция маршрутов.** `/menu` и `/<name>` совпадают по форме, но Werkzeug сортирует
  ...
  дефолтного `string`.
* `/menu/abc` **не** даёт 404 и **не** вызывает `dish()`: конвертер `int` не совпадает, и правило
  отбрасывается — но путь всё ещё подходит под `/<category>/<sub>`, который и отработает
  (`category=menu`, `sub=abc`). Отсюда общее правило: несовпадение конвертера это не ошибка,
  а переход к следующему правилу. 404 будет, только если не подошло НИ ОДНО.
