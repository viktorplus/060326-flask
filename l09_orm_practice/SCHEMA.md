...
* **Проверка удаления обращалась к удалённому объекту и стояла вне `if`.**
  ...
  не найден, — то есть по `None`. Теперь `id` запоминается ДО удаления,
  а проверка перенесена внутрь `if`.
...
* **`session.delete()` + `commit()` переводит объект в состояние detached** — но,
  вопреки распространённому мнению, поля остаются читаемыми. Проверено запуском:
  и `user.id`, и `user.name` после удаления возвращают значения, потому что уже
  загруженные данные остаются в самом объекте. `DetachedInstanceError` даст только
  обращение к полю, которое было обесценено: перечитать его неоткуда, объект
  больше не привязан к сессии. Сохранять `id` заранее стоит не из-за исключения,
  а ради читаемости.
